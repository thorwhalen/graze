...
def test_discover_exceptions_with_files_cache():
    ...
    try:
        ...
        exceptions_file.write_text(json.dumps({
            'http://example.com/data': '/path/to/data.txt'
        }))
...
def test_wrap_with_exceptions_warns_on_missing_files():
    ...
    cache = {'url1': b'data'}
    exceptions = {
        'url2': '/nonexistent/file.txt',
        'url3': '/another/missing.txt'
    }
...
def test_graze_cache_dict_with_url_to_cache_key_transform():
    ...
    try:
        ...
        wrapped = graze_cache(cache, exceptions=exceptions, url_to_cache_key=simple_transform)
...
def test_add_exception_basic():
    ...
    try:
        ...
        assert Path(exceptions['http://example.com/data']).resolve() == Path(data_file).resolve()
    finally:
        os.unlink(data_file)
        exceptions_file = Path(cache_dir) / '_exceptions.json'
        if exceptions_file.exists():
